...
battery_widget:set_font("Noto Sans 10") 

function update_battery_widget()
    local status = io.popen("acpi -b"):read("*line")
    local battery_percentage = status:match("(%d+)%%")
    local charging_status = status:match("(%a+),")
    ...
    battery_widget.text = "🔋 "  .. battery_percentage .. "% " .. emoji
end
...
gears.timer {
    timeout = 2,
    autostart = true,
    callback = function() update_battery_widget() end
}
...
-- Define your wallpaper function
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create the volume widget (assuming you have a volume.lua file)
    local volume = require("volume")
    local volume_widget = volume.getwidget()
